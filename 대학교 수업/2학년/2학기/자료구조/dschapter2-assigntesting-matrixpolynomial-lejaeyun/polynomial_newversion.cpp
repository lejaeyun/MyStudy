...
public:
	...
	friend Polynomial& operator+(const Polynomial& m, const Polynomial& n) {
		Polynomial Tmp;
		int max = 0;
		...
		for (int i = 0; i < max; i++)
		{
			float a;
			int b;
			a = m.termArray[i].coef + n.termArray[i].coef;
			b = m.termArray[i].exp + n.termArray[i].exp;
			Tmp.NewTerm(a, b);
		}
	}
	friend Polynomial& operator-(const Polynomial& m, const Polynomial& n) {
...
istream& operator >> (istream& stream, Polynomial& m) {
	for (int i = m.start; i < m.finish; i++) {
		float a;
		int b;
		stream >> a;
		stream >> b;
