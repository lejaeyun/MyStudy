//10장-15-3.TwinThing.cpp [PRATA2005] 10.4.3 template을 이용한 Container class 표현
#include <iostream>
#include <string>
using namespace std;
...
int main()
{
	...
	for (int i = 0; i < 4; i++)
	   cout << course[i].access_a() << course[i].access_b() << endl;
	course[2].access_a() = "C programming";
	course[2].access_b() = 2;
	cout << course[2].access_a() << course[2].access_b() << endl;
	system("pause");
	return 0;
}